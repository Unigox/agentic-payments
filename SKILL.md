...
11. When asking the user to fetch that signing key from unigox.com, keep the browser-login guidance consistent with the auth path they actually used. For TON auth, explain the TonConnect paths: they can scan a fresh UNIGOX TonConnect QR in the wallet, paste a fresh `tc://` TonConnect link here, send a fresh local screenshot of the visible UNIGOX TonConnect QR when the runtime can pass an image path to the runner, or log in to unigox.com directly from their mobile or desktop TON wallet. Exception: if the skill generated the TON wallet locally, do not tell the user to scan that QR in a separate wallet app. Tell them to send the fresh QR or `tc://` link back to the agent so the agent can approve the browser login with the stored TON key on this machine. For EVM auth, do not mention TonConnect. Tell them to use the EVM wallet-connection flow on unigox.com for the same wallet, complete any WalletConnect-style QR / deep-link / browser-extension approval there, and then export the signing key from settings. If the skill generated the EVM wallet locally, remind them they can say `export this wallet` first, import that isolated wallet into their browser wallet, and then use the website’s EVM wallet-connection flow.
