...
- **EVM browser login helper from a fresh `wc:` link or fresh QR screenshot** — if the user is already on `unigox.com` and the site is showing a fresh WalletConnect login request for the EVM wallet, the skill can consume either the copyable `wc:...` link or a fresh local QR screenshot, then approve that live browser-login request locally with the stored EVM login wallet. This requires a local WalletConnect / Reown project id on the machine (`WALLETCONNECT_PROJECT_ID`, `REOWN_PROJECT_ID`, or `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID`). The screenshot path only works when the adapter/runtime can supply a local image path to the runner.
