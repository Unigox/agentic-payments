...
> Which UNIGOX sign-in path should I set up: **EVM wallet connection**, **TON wallet connection**, **create a dedicated EVM wallet on this device**, or **create a dedicated TON wallet on this device**? If neither wallet path is ready yet, we can still use **email OTP** for onboarding or recovery.
...
- **Generated EVM login wallet** — after email OTP, the skill can create a dedicated local EVM login wallet with cryptographic randomness, link it to the UNIGOX account, and persist it as `UNIGOX_EVM_LOGIN_PRIVATE_KEY` so the user never has to paste the login key manually.
...
- **Generated TON login wallet** — after email OTP, the skill can create a dedicated TON wallet locally with cryptographic randomness, link it to the UNIGOX account, and persist `UNIGOX_TON_PRIVATE_KEY`, `UNIGOX_TON_ADDRESS`, and `UNIGOX_TON_WALLET_VERSION` so the user does not have to paste TON login secrets manually.
