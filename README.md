...
- **TON wallet auth** — for new setups, use the exact raw TON address from the wallet plus the TON private key / secret key for that same wallet. The agent signs the frontend TON proof locally and uses the raw address as the source of truth, so it does not guess the wrong wallet version. Legacy `UNIGOX_TON_MNEMONIC` env-based installs still work, but new chat onboarding should not collect mnemonics.
