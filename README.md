...
- **TON wallet auth** — for new setups, use the exact raw TON address from the wallet plus the TON private key / secret key for that same wallet. The agent signs the frontend TON proof locally, treats the raw address as the source of truth, tries the supported TON wallet versions until one matches that exact address, and stores the matched derivation as `UNIGOX_TON_WALLET_VERSION` for later runs. Legacy `UNIGOX_TON_MNEMONIC` env-based installs still work, but new chat onboarding should not collect mnemonics.
