...
- When the only remaining blocker is the exported UNIGOX signing key, keep the browser-login guidance aligned with the login path. For TON auth, explain the TonConnect paths: scan a fresh UNIGOX TonConnect QR in the wallet, copy the fresh `tc://` TonConnect link into the wallet if the site shows that instead of a QR, or log in on unigox.com directly from the user's mobile or desktop TON wallet. If the skill generated the TON wallet locally, change that guidance: the user should send the fresh QR or `tc://` link back to the agent, and the agent should approve the browser login locally with the stored TON key instead of telling the user to scan it in another wallet app. For EVM auth, do not mention TonConnect. Tell the user to use the website's EVM wallet-connection flow for the same wallet, complete any WalletConnect-style QR / deep-link / browser-extension approval there, and then export the signing key from settings. If the skill generated the EVM wallet locally, tell the owner they can say `export this wallet`, import that isolated wallet into their browser wallet, and then use the website EVM wallet-connection flow.
