...
const SAMPLE_WC_LINK = "wc:266081884b684924211ce2e68e0808e18c6c7f82cda45dcf59837aca50979a05@2?relay-protocol=irn&symKey=6fba76027bd0bc22245b7c5223201ee3e07ac3856e02224ab0c6a4a7f498abe2";
...
test("decodeWalletConnectUriFromImagePath extracts a wc link from a QR screenshot", async () => {
  const file = path.join(fs.mkdtempSync(path.join(os.tmpdir(), "walletconnect-qr-")), "wc-link.png");
  const buffer = await QRCode.toBuffer(SAMPLE_WC_LINK, { type: "png", width: 512, margin: 1 });
  fs.writeFileSync(file, buffer);

  const decoded = await decodeWalletConnectUriFromImagePath(file);
  assert.equal(decoded, SAMPLE_WC_LINK);
});
